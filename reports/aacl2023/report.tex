...
We encourage researchers to utilize context-sensitive vectors such as RoBERTa Tagalog (or other BERT variants) when training models from this corpora as demonstrated in Table \ref{table:results_baseline}.
...
In order to properly evaluate on WikiANN, we reannotated the test dataset using the same annotation guidelines as done in Section \ref{sec:annotation_setup}.
Table \ref{table:results_wikiann} reports the F1-score across three trials.

Additionally, we also measured the overlap between the reannotated WikiANN to the original test dataset to demonstrate the gap between gold-standard annotations done by native speakers against a silver-standard one.
Table \ref{table:results_gold} reports these results.

\subfile{tables/results_gold}

Results suggest that models built from the \tlunified{} corpus is more performant than with WikiANN.
We hypothesize that the gold-standard nature of \tlunified{} led to better performance than WikiANN, which predominantly consists of text fragments and low-quality annotations.
...
These findings are consistent with \citet{Wang2022WhatLM}, where they showed that although decoder-only LMs exhibit strong zero-shot generalization, they are still outperformed by models trained via masked language modeling followed by multitask finetuning.

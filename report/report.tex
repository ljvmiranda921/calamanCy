...
This work has two main contributions: (1) an open-source toolkit containing general-purpose multitask pipelines with out-of-the box support for common NLP tasks, and (2) comperehensive benchmarks on several Tagalog core NLP tasks.
...
In addition, scispaCy \citep{Neumann2019ScispaCyFA} and medspaCy \citep{Eyre2021LaunchingIC} focus on biomedical and scientific text.
% Such toolkits include DaCy for Danish \citep{Enevoldsen2021DaCyAU} and HuSpaCy for Hungarian \citep{Orosz2022HuSpaCyAI}.
...
Using spaCy as a foundation is optimal, given its popularity and tight integration with other frameworks like HuggingFace transformers \citep{Wolf2019HuggingFacesTS}.
However, no tool has existed for Tagalog until now.
We aim to fill this development gap and serve the needs of Tagalog NLP through calamanCy.
...
In addition, Tagalog has two Universal Dependencies (UD) treebanks, Tagalog Reference Grammar (TRG) \citep{Samson2018TRG} and Ugnayan \citep{Aquino2020ParsingIT}, both with POS tags and relational structures.
...
We used TLUnified-NER to train the NER component and trained the dependency parser and POS tagger using the combined treebanks.
...
However, for dependency parsing and POS tagging, the combined treebank is still small ($\ll$ 20k tokens), so we followed the Universal Dependencies data split guidelines and performed 10-fold cross-validation \citep{Nivre2020UniversalDV}.
...
They used a metric \citep{Agic2017CrossLingualPS} based on the World Atlas for Language Structures \citep{Haspelmath2005WALS}.

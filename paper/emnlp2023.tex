...
For domain-specific data, there is medspaCy for clinical text \citep{Eyre2021LaunchingIC} and scispaCy for scientific articles \citep{Neumann2019ScispaCyFA}.
...
We evaluated on the following tasks and datasets:
\begin{itemize}
  \item \textbf{Text classification} For binary text classification, we used the \textit{Hatespeech} dataset \citep{Cabasag2016HatespeechIP} that contains 10k tweets during the 2016 Philippine Presidential Election labeled as hate speech or non-hate speech.
  For multilabel text classification, we used the \textit{Dengue} dataset \citep{Livelo2018IntelligentDI} that also contains tweets on the dengue 
